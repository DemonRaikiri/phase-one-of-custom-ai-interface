...
# What the "Hey Vegeta" wake phrase opens.  These use the installed Windows
# applications rather than browser shortcuts.
OPEN_SPOTIFY_ON_WAKE = True
OPEN_BRAVE_ON_WAKE = True
OPEN_CODEX_ON_WAKE = True
OPEN_DASHBOARD_ON_WAKE = True
# Override this in .env if the Codex package id changes after an app update.
CODEX_APP_ID = os.environ.get(
...
# Local, offline speech recognition for the wake phrase.  The Vosk model lives
# in `models/vosk-model-small-en-us-0.15` beside this script.
WAKE_PHRASE = "hey vegeta"
...
WAKE_MODEL_DIR = Path(__file__).resolve().parent / "models" / "vosk-model-small-en-us-0.15"

# Support the shared project configuration one level above this script, while
# allowing an optional script-local file to override individual settings.
...
def _wake_recognizer() -> KaldiRecognizer:
    if not WAKE_MODEL_DIR.is_dir():
        raise RuntimeError(
            f"Wake-word model is missing: {WAKE_MODEL_DIR}. "
            "Download and unpack the Vosk small English model there."
        )
    SetLogLevel(-1)
    model = Model(str(WAKE_MODEL_DIR))
    return KaldiRecognizer(model, SAMPLE_RATE, json.dumps([*WAKE_PHRASE_VARIANTS, "[unk]"]))


def _heard_wake_phrase(recognizer: KaldiRecognizer, block: np.ndarray) -> bool:
    samples = np.clip(block.reshape(-1), -1.0, 1.0)
    pcm = (samples * 32767).astype(np.int16).tobytes()
    payload = recognizer.Result() if recognizer.AcceptWaveform(pcm) else recognizer.PartialResult()
    text = json.loads(payload).get("text") or json.loads(payload).get("partial") or ""
    return any(phrase in text.lower() for phrase in WAKE_PHRASE_VARIANTS)
...
def _quiet_popen(args: list[str]) -> None:
...
def open_spotify() -> None:
    ...
    try:
        if exe:
            ...
            log.warning("Spotify was not found.")
    except OSError as e:
        log.warning("Could not launch Spotify: %s", e)


def open_brave() -> None:
    exe = _brave_executable()
    if not exe:
        log.warning("Brave was not found.")
        return
    try:
        _quiet_popen([exe])
    except OSError as e:
        log.warning("Could not launch Brave: %s", e)


def open_codex() -> None:
    if sys.platform != "win32":
        log.warning("Codex app launching is currently configured for Windows only.")
        return
    try:
        _quiet_popen(["explorer.exe", fr"shell:AppsFolder\\{CODEX_APP_ID}"])
    except OSError as e:
        log.warning("Could not launch Codex: %s", e)
...
def main() -> int:
    ...
    wake_recognizer = _wake_recognizer()
    ...
    log.info("Listening for the wake phrase: %r", WAKE_PHRASE)
    if SONG_URI.strip():
    ...
    try:
        with sd.InputStream(
            ...
            while True:
                ...
                if not welcome_sequence_done and _heard_wake_phrase(wake_recognizer, data):
                    welcome_sequence_done = True
                    wake_recognizer.Reset()
                    log.info("Wake phrase detected: %s", WAKE_PHRASE)
